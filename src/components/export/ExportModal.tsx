...
import { exportDataset, selectSaveFolder, selectSaveFile } from "@/lib/tauri";
...
export function ExportModal({ isOpen, onClose }: ExportModalProps) {
  ...
  const willExport = onlyCaptioned ? captionedCount : images.length;
        ...
        <div className="space-y-4 p-4">
          ...
          <div className="rounded bg-surface p-3 text-sm">
            <p className="text-gray-400">
              {images.length} total images • {captionedCount} captioned
            </p>
          ...
          <div>
            <label className="mb-1 block text-sm text-gray-400">
              {asZip ? "Save ZIP as" : "Export to folder"}
            </label>
            <div className="flex gap-2">
              <input
                type="text"
        ...
        <div className="flex justify-end gap-2 border-t border-border px-4 py-3">
          ...
          {!result?.success && (
            <button
              ...
              disabled={!destPath || exportMutation.isPending}
